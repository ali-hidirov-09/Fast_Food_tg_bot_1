...
def send_main_menu(context, chat_id, lang_id, message_id=None):
    buttons = [
        [KeyboardButton(text=globals.BTN_ORDER[lang_id]), KeyboardButton(text=globals.BTN_MY_ORDERS[lang_id])],
        [KeyboardButton(text=globals.BTN_ABOUT_US[lang_id]), KeyboardButton(text=globals.BTN_SETTINGS[lang_id])]
    ]
    if message_id:
