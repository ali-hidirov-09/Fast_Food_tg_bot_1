...
def inline_handler(update, context):
    ...
    if data_sp[0] == "category":
        if data_sp[1] == "product":
            if data_sp[2] == "back":
                ...
                if len(data_sp) == 4:
                    ...
                    text += f"\n{globals.ALL[db_user['lang_id']]}: {total_price} тенге"
        ...
        if len(data_sp) == 2 and data_sp[1] == "clear":
            ...
            buttons = [
                [
                    InlineKeyboardButton(text=f'{globals.ZAKAZ_OL[db_user["lang_id"]]}', callback_data="order"),
                    InlineKeyboardButton(text=f'{globals.ZAKAZ_OL_OL[db_user["lang_id"]]}', callback_data="cart_clear")
                ],
                [InlineKeyboardButton(text=f'{globals.ZAKAZ_OL_OL_OL[db_user["lang_id"]]}', callback_data="cart_back")],
            ]
            query.message.edit_reply_markup(
                reply_markup=InlineKeyboardMarkup(
                    inline_keyboard=buttons
        ...
        if len(data_sp) > 1 and data_sp[1] == "payment":
            if data_sp[2] == '1':
                context.user_data['payment_type'] = 1
            elif data_sp[2] == '2':
                context.user_data['payment_type'] = int(data_sp[2])
                query.message.reply_text(
                    text="Pulni  +77088613884  raqamiga jo'nating"
                )
            ...
            query.message.edit_reply_markup(
                reply_markup=InlineKeyboardMarkup(
                    [[
                        InlineKeyboardButton(text=f"{globals.PUL[db_user['lang_id']]}", callback_data="order_payment_1"),
                        InlineKeyboardButton(text=f"{globals.PULL[db_user['lang_id']]}", callback_data="order_payment_2"),
                    ]]
                )
            )
