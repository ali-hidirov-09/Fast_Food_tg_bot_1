...
def message_handler(update, context):
    ...
    if state == 0:
        ...
        if not db_user["lang_id"]:
            ...
            elif message == globals.BTN_LANG_KZ:
...
################## lesson-4 ###################
        ...
        if message == globals.BTN_ORDER[db_user['lang_id']]:
            ...
            if context.user_data.get("carts", {}):
                ...
                update.message.reply_text(
                    text=f"<b>{globals.MALUM[db_user['lang_id']]}</b>\n\n"
                         f"👤 <b>{globals.NAME[db_user['lang_id']]}</b> {db_user['first_name']} {db_user['last_name']}\n"
                         f"📞 <b>{globals.NUMBER[db_user['lang_id']]}</b> {db_user['phone_number']} \n\n"
                         f"📥 <b>{globals.ZAKAZZ[db_user['lang_id']]}</b> \n"
                         f"{text}",
                    parse_mode='HTML'
                )
            ...
            buttons = [
                [KeyboardButton(text=globals.BTN_LANG_UZ), KeyboardButton(text=globals.BTN_LANG_RU), KeyboardButton(text=globals.BTN_LANG_KZ)]
            ]
            update.message.reply_text(
